...
print(paste("Cellranger-atac data:",data_folder,sep=""))
##output data
print(sample)
...
atac_counts <- Read10X_h5(paste(data_folder,"/outs/raw_peak_bc_matrix.h5",sep=""))
...
metadata <-read.csv(file=paste(data_folder,"/outs/singlecell.csv",sep=""), header = TRUE, row.names = 1)
fragment.path <- paste(data_folder,"/outs/fragments.tsv.gz",sep="")
...
matrix.counts <- FeatureMatrix(
  fragments = Fragments(panc@assays$peaks),
...
blacklist_counts <- CountsInRegion(panc, assay = 'X500peaksMACS2', region = blacklist_hg38_unified) %>% data.frame(); colnames(blacklist_counts) <- 'blacklist_region_fragments'
...
#### after QC
pdf(paste(outputpath,"/After_QC_in_sample_",sample, "_ATAC_MACS2.pdf", sep=""), width=17, height=9)
VlnPlot(object = panc, features = c('pct_read_in_peaks_500MACS2', 'peak_region_fragments_500MACS2',  'TSS.enrichment', 'nucleosome_signal'), ncol = 4)
dev.off()
...
#Check if first LSI-component correlated with the sequencibg depth. 
...
##################################################
panc <- RunUMAP(panc, reduction = 'lsi', dims = opt$pc_first:opt$pc_num, 
                reduction.name = "atac.umap", reduction.key = "atacUMAP_")
panc <- FindNeighbors(object = panc, reduction = 'lsi', dims = opt$pc_first:opt$pc_num)
panc <- FindClusters(object = panc, verbose = FALSE, algorithm = 3)


p2 <- DimPlot(panc, reduction = "atac.umap", group.by = "seurat_clusters", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("ATAC")

pdf(paste(outputpath,"/",sample,"_2_Dimplots.pdf",sep=""),height=6,width=6)
...
#Save object
saveRDS(panc,file = paste(outputpath,"/",sample, "_processed_atac.rds", sep=""))
