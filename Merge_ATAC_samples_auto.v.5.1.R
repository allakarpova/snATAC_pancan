...
if(file.exists(paste0(length(samples.id),'_recentered_final.filtered.',add_filename,'.tsv'))) {
  ...
  fwrite(recentered_non_olap,paste0(length(samples.id),'_recentered_nonOverlapping.filtered.',add_filename,'.tsv'),
         sep='\t',row.names=FALSE)
  rm(recentered_non_olap)
  gc()
  
  
  pairs_all=cbind(all_peaks[olap1$queryHits,c('seqnames','start','end', 'score')],
                  'row_1' = olap1$queryHits,
                  all_peaks[olap1$subjectHits,c('seqnames','start','end', 'score')],
                  'row_2' = olap1$subjectHits)
  colnames(pairs_all)=c('chr_1','st_1','en_1','score_1',
                        'row_1',
                        'chr_2','st_2','en_2','score_2',
                        'row_2')
  rm(olap1)
  gc()
  ...
  pairs_all = pairs_all[,c(4,5,9,10)]
  #prepare for parallelization
  pairs_all <- split(pairs_all, pairs_all$chr_1)
  
  library(doParallel)
  registerDoParallel(cores=30)
  all_st <- foreach(pairs=pairs_all) %dopar% {
    all_st_chr=NULL
    for (peak.row in unique(pairs$row_1)) {
        all_st_chr=rbind(all_st_chr,
                         all_peaks[peak.row,])
    }
    return(all_st_chr)
  }
  ...
  recentered_final=rbind(recentered_non_olap,
                         all_st_f,
                         olap)
  ...
  #remove big things
  rm(recentered_non_olap, all_st_f, all_st,all_st_1,olap,pairs_all)
  gc()
}
#########################################################################
