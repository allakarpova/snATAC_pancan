...
panc <- SCTransform(panc, verbose = FALSE) %>% 
  RunPCA() %>% 
  RunUMAP(dims = 1:opt$pc_num, reduction.name = 'rna.umap', reduction.key = 'rnaUMAP_')
...
panc <- RunSVD(panc)
panc <- RunUMAP(panc, reduction = 'lsi', dims = opt$pc_first:opt$pc_num, reduction.name = "atac.umap", reduction.key = "atacUMAP_")
...
p1 <- DimPlot(panc, reduction = "rna.umap", group.by = "seurat_clusters", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("RNA")
