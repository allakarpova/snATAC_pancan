...
option_list = list(
  ...
  make_option(c("-a","--assay"),
              type="character",
              default='ATAC_immune',
              help = "X500peaksMACS2 or peaksinters or pancan or ATAC_immune",
              metavar="character"),
  make_option(c("--annotation"),
              type="character",
              default='/diskmnt/Projects/snATAC_primary/PanCan_ATAC_data_freeze/v2.0/snATAC/merged_no_recalling_upd/Annotations.EnsDb.Hsapiens.v86.rds',
              help = "path to EnsDb.Hsapiens.v86 based annotation",
              metavar="character")
...
annotations <- readRDS(opt$annotation)

dir.create(out_path, showWarnings = F)
setwd(out_path)

cat('opening integrated object with old peaks object \n')
int <- readRDS(input.path)


#call peaks
if (file)
recentered_final <- call_recenter_filter_peaks(int, add_filename, macs2_path)
...
frag <- Fragments(int@assays[[assay.towork]])
...
int[['ATAC_immune']] <- CreateChromatinAssay(counts = matrix.counts,
                                                 fragments=frag,
                                                 annotation = annotations)
DefaultAssay(int)<-'ATAC_immune'
int <- DietSeurat(int, assays = 'ATAC_immune')

peak.counts <- colSums(GetAssayData(object = int, assay = 'ATAC_immune', slot = "counts"))
total_fragments_cell <- int$passed_filters
frip <- peak.counts / total_fragments_cell
int <- AddMetaData(object = int, metadata = frip, col.name = 'pct_read_in_peaks_ATAC_immune')
int <- AddMetaData(object = int, metadata = peak.counts, col.name = 'peak_region_fragments_ATAC_immune')


#normalize merged object
int <- int %>% FindTopFeatures(min.cutoff = 10) %>%
  RunTFIDF() %>%
  RunSVD() %>%
  RunUMAP(reduction = "lsi", dims = 2:50)

DimPlot(int, group.by = "Cancer", cols = 'Spectral') + 
  DimPlot(int, group.by = "cell_type.harmonized.cancer") +
  DimPlot(int, group.by = "Chemistry")
ggsave(paste0(add_filename, '_merged_dimplots.pdf'), width = 17, height = 4.5)

saveRDS(int, paste0(add_filename, '_merged.rds'))
...
atac.split <- SplitObject(int, split.by = 'Chemistry')
...
p1 <- DimPlot(int, group.by = "Chemistry")
...
p1 <- DimPlot(int, group.by = "cell_type.harmonized.cancer")
...
p1 <- DimPlot(int, group.by = "Cancer", cols = 'Spectral')
(p1 + ggtitle("Merged")) | (p2 + ggtitle("Integrated"))
