...
option_list = list(
  make_option(c("-i", "--input.object"),
              ...
              metavar="character"),
  make_option(c("-s", "--satpathy.object"),
              type="character",
              default=NULL, 
              help="path to Satpathy merged object",
              metavar="character"),
...
annotations <- readRDS('/storage1/fs1/dinglab/Active/Projects/ATAC_immune/Annotations.EnsDb.Hsapiens.v86.rds')
satpathy.path <- opt$satpathy.object

dir.create(out_path, showWarnings = F)
setwd(out_path)

if (!file.exists(paste0(add_filename, '_merged.rds'))) {
  cat('opening object \n')
  ...
  satpathy.obj <- readRDS(satpathy.path)
  satpathy.obj$Chemistry <- 'snATAC'
  ...
  cat('Merging \n')
  merged <- merge(x = int, y = satpathy.obj)
  ...
  merged <- readRDS(paste0(add_filename, '_merged.rds'))
}
...
p2 <- DimPlot(integrated, group.by = "Piece_ID")
p1 <- DimPlot(merged, group.by = "Piece_ID")
(p1 + ggtitle("Merged")) | (p2 + ggtitle("Integrated"))
ggsave(paste0(add_filename, '_Piece_ID.pdf'), width = 18, height = 6)
