...
Idents(panc) <- panc@meta.data[[cell_column]]
print(head(Idents(panc)))

dim(panc)
cat('subsetting \n')
panc.my <- subset(x = panc, idents = "Doublet", invert = TRUE)
dim(panc.my)
...
saveRDS(panc.my, paste0('Immune_object_old_peaks_normalized_', add_filename, '.rds'))
cat('calling new peaks \n')
...
saveRDS(panc.my, paste0('Immune_object_new_peaks_', add_filename, '.rds'))

##########################
##### Integration #######
##########################

atac.split <- SplitObject(panc.my, split.by = 'Chemistry')
...
panc.my <- panc.my %>% FindTopFeatures(min.cutoff = 10) %>%
  RunTFIDF() %>%
  RunSVD() %>%
  RunUMAP(reduction = "lsi", dims = 2:50)
saveRDS(panc.my, paste0(add_filename, '_MERGED.rds'))
...
ggsave(glue::glue('{add_filename}_{cell_column}.pdf'), width = 15, height = 4.5)
