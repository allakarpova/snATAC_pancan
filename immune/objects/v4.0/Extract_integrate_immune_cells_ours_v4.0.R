...
cancers <- c('BRCA', 'ccRCC', 'GBM', 'CRC', 'HNSCC', 'MM', 'CESC', 'OV', 'UCEC', "PDAC")
...
atac <- map(paths, function(p) {
  ...
  print(ct)
  stopifnot(!is.na(ct))
  obj$is_immune <- grepl('Plasma|B-cells|T-cell|DC|Macro|Mast|Microglia|NK|Treg|Immune|Gran', as.character(unlist(obj[[cell_column]])))
  cat('subsetting\n')
