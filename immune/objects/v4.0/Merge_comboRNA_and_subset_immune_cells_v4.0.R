...
if(!file.exists(paste0(length(samples.id),"_snRNA_combo_Merged_immune_",add_filename,".rds"))) {
  paths <- foreach (s = samples.id, df = samples.df) %do% {
    print(s)
    p <- list.files(path = df, full.names = T, 
                    pattern = paste0(str_split_fixed(s, '_',2)[2],'.*rds'), all.files = F, recursive = T)
    print(length(p))
    return(p)
  }
  ...
  int$is_immune <- grepl('Plasma|B-cells|T-cell|DC|Macro|Mast|Microglia|NK|Treg|Immune|Gran', as.character(unlist(int[[cell_column]])))
  ...
  int <- readRDS(paste0(length(samples.id),"_snRNA_combo_Merged_immune_",add_filename,".rds"))
}

#NOW extract lineages

int$is_Tcell_lin <- grepl('T-cell|NK|Treg', as.character(unlist(int[['cell_type.immune']])))
int$is_Bcell_lin <- grepl('B-cell|Plasma', as.character(unlist(int[['cell_type.immune']])))
int$is_Myeloid_lin <- grepl('Macro|DC|Mast|Microgl', as.character(unlist(int[['cell_type.immune']])))
...
conditions %>% walk (function(column) {
  ...
  dimplot=DimPlot(object = int.sub, label = TRUE, group.by = "cell_type.immune")
  pdf(paste0('Dimplot_cell_type.immune_', column,"_snRNA_combo_Merged_immune_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
