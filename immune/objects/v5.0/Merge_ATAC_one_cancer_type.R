...
load_peaks <- function(sample){
  input.path <- samples.folders[sample]
  peaks=fread(paste0(input.path,"/recentered_final.filtered",sample,".tsv"))
  peaks$Sample=sample
...
piece.id <- samples$Piece_ID %>% as.character()
samples.type <- samples$`Data Type` %>% as.character()
samples.folders <- samples$`Data folder`; names(samples.folders) = samples.id
...
if (file.exists(paste0(length(samples.id),"_snATAC_Merged_not_normalized_",add_filename,".rds"))) {
  ...
  cat ('Merging\n')
  combined <- merge(x = atac[[1]], y = atac[-1], add.cell.ids = paste0(cancer.type, '_', piece.id))
...
if(file.exists(paste0('peaks/', length(samples.id),'_All_recentered_final.',add_filename,'.tsv'))) {
  cat('loading ready-to-go peaks \n')
  pancan_all_peaks <- fread( paste0('peaks/',length(samples.id),'_All_recentered_final.',add_filename,'.tsv'), data.table = T)
  ...
  recentered_final.cancer <- foreach(ap=all_peaks.cancer, cancer.type=names(all_peaks.cancer))  %do% {
    ap <- ap[order(score.norm, decreasing = T), ] # order peaks by normalized scores, this is essential for filtering out overlapping peaks
  ...
  ########
  pancan_all_peaks <- rbindlist(recentered_final.cancer)
  fwrite(pancan_all_peaks, paste0('peaks/',length(samples.id),'_All_recentered_final.',add_filename,'.tsv'),sep='\t',
               row.names=FALSE)
  
}
#########################################################################

recentered_p=StringToGRanges(unique(pancan_all_peaks$new_peak), sep = c("-", "-"))
...
peak.number <- length(unique(pancan_all_peaks$new_peak))
...
combined[['pancan']] <- CreateChromatinAssay(counts = matrix.counts,
                                                     annotation = annotations,
                                                     genome = 'hg38',
                                                     fragments = frag)
# remove ATAC assay
DefaultAssay(combined)<-'pancan'
