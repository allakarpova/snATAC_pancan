...
integrated <- integrated %>%
  FindNeighbors(reduction = "mnn", dims = 2:50) %>%
  FindClusters(verbose = TRUE, resolution = 2) %>%
  RunUMAP(reduction = "mnn", dims = 2:50)

print(integrated@reductions)
saveRDS(integrated, paste0('PanImmune_fastMNN_integrated_object_new_peaks_', add_filename, '_chemistry_data.source.rds'))

fwrite(cbind(Embeddings(integrated, reduction='umap'), integrated@meta.data), 
       paste0('PanImmune_fastMNN_integrated_object_new_peaks_', add_filename, '_chemistry_data.source_matadata.tsv'),
       sep='\t', row.names = T)
