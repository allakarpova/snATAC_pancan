...
##### START
input <- '/diskmnt/primary/published_data/panatac_immune_scATAC_hg38/sorted/'
...
#remove Mono_T mixtures and CD4Mem_CD8Naive mixtures
samples <- samples[!grepl('Mono_T|MonoT|CD4Mem_CD8Naive', samples)]
frag.files <- frag.files[!grepl('Mono_T|MonoT|CD4Mem_CD8Naive', frag.files)]
...
keep.them <- lapply(samples, function(x) {
  !file.exists(paste0('/diskmnt/Projects/snATAC_primary/02_atac_rds_signca/external_Satpathy/', x, '/', x, '_processed_multiomic.rds'))
})
keep.them <- unlist(keep.them)
print(keep.them)
frag.files <- frag.files[keep.them]
samples <- samples[keep.them]
