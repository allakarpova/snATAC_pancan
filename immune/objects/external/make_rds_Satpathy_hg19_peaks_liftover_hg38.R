...
add_blacklist <- function(obj, tb) {
  ...
  blacklist.overlaps <- countOverlaps(frag.gr, blacklist_hg19)
...
##### START
input <- '/diskmnt/primary/published_data/panatac_immune_scATAC_hg19'
frag.files <- list.files(path = input, pattern = '*fragments.tsv.gz$')

samples <- str_replace(string = frag.files, pattern = '_fragments.tsv.gz', replacement = '')
...
cat('Making tiles\n')
gr.chr <-  GRanges(seqnames = chromsize$V1,ranges= IRanges(start = rep(0, 455), end = chromsize$V2))
tiles <- GenomicRanges::tile(gr.chr,width = 500000)
stand.chrom <- c(paste0('chr', 1:22), 'chrX')
tiles <- unlist(tiles)
tiles <- tiles[seqnames(tiles) %in% stand.chrom,]

annotations.hg19 <- GetGRangesFromEnsDb(ensdb = EnsDb.Hsapiens.v75,  standard.chromosomes = TRUE)
seqlevelsStyle(annotations.hg19) <- 'UCSC'
genome(annotations.hg19) <- "hg19"
annotations.hg38 <- GetGRangesFromEnsDb(ensdb = EnsDb.Hsapiens.v86,  standard.chromosomes = TRUE)
seqlevelsStyle(annotations.hg38) <- 'UCSC'
genome(annotations.hg38) <- "hg38"
#tiles.dt <- tiles %>% as.data.table()
#features <- paste(tiles.dt$seqnames, tiles.dt$start, tiles.dt$end, sep ='-')

walk2(frag.files, samples, function(cur.fr, sample) {
  output.path <- '/diskmnt/Projects/snATAC_primary/02_atac_rds_signca/external_Satpathy'
  sample.out <- paste0(output.path, '/',sample)
  dir.create(sample.out)
  setwd(sample.out)
  #cur.fr <- frag.files[[1]]
  ...
  fragments.hg19 <- CreateFragmentObject(
    path = cur.fr,
    cells = atac.cells, 
  ...
  peaks <- CallPeaks(
    object = object,
    macs2.path='/diskmnt/Projects/Users/allakarpova/Tools/anaconda3/envs/signac1.4/bin/macs2'
  )
  
  hg19_hg38 <- rtracklayer::import.chain("/diskmnt/Datasets/Reference/liftOver.chain/hg19ToHg38.over.chain.gz")
  peaks_hg38 <- rtracklayer::liftOver(x = peaks, chain = hg19_hg38)
  
  # remove peaks on nonstandard chromosomes and in genomic blacklist regions
  peaks_hg38 <- keepStandardChromosomes(peaks_hg38, pruning.mode = "coarse")
  peaks_hg38 <- subsetByOverlaps(x = peaks_hg38, ranges = blacklist_hg38_unified, invert = TRUE)
  
  all_peaks=as.data.table(peaks_hg38)
  ...
  fr.hg38 <- paste0('/diskmnt/primary/published_data/panatac_immune_scATAC_hg38/', sample, '_fragments.tsv.gz')
  fragments.hg38 <- CreateFragmentObject(
    path = fr.hg38,
    cells = atac.cells, 
    validate.fragments = TRUE
  )
  ...
  frag.tb <- fread(fr.hg38, col_names = c('seqnames', 'start', 'end', 'barcode', 'N'))
  ...
  saveRDS(object, paste0(sample, "_processed_multiomic.rds"))
