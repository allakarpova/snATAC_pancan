...
walk2(frag.files, samples, function(cur.fr, sample) {
  ...
  frag.tb <- fread(cur.fr, col_names = c('seqnames', 'start', 'end', 'barcode', 'N'))
  ...
  colnames(chromsize)=c('seqnames','chr_length')
  all_peaks=merge(all_peaks,chromsize,all.x=TRUE)
  ...
  cat('making counts\n')
  matrix.counts <- FeatureMatrix(
    fragments = Fragments(object@assays$ATAC),
