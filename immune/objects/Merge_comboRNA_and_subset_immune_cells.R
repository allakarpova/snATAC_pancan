...
int <- merge(x = atac[[1]], y = atac[2:length(samples.id)], add.cell.ids = samples.id)  
...
int$is_immune <- grepl('Plasma|T-cell|DC|Macro|Mast|Microglia|NK|Treg|Immune|Kuppfer|Lympho|Neutro|Gran', as.character(unlist(int[[cell_column]])))
...
int <- FindClusters(object = int, verbose = FALSE)
