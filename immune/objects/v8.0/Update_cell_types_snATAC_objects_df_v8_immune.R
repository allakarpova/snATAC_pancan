...
option_list = list(
  ...
  make_option(c("-i", "--meta.input"),
              type="character",
              default="/diskmnt/Projects/snATAC_primary/04_celltyped_rds/v10.0_for_immune", 
...
samples <- samples %>% dplyr::filter(`Disease Type` == 'PBMC' | (`Disease Type` == 'ccRCC' & `Data Type` == '10x_SC_Multi_ATAC_SEQ'))
...
for (i in 1:nrow(samples)) {
  ...
  print(s)
  if (!do.all.over) {
    if (file.exists(paste0(s,'_upd_v8_cellTyped.meta.data'))) { # dont do updating if its already updated
      next()
    }
  }
  ...
  ggsave(paste0(s,'_before_update_to_v8.pdf'), width = 12, height = 6, useDingbats = F)
  ...
  fwrite(input.obj@meta.data, paste0(s,'_upd_v8_cellTyped.meta.data'), sep = '\t', row.names = T)
