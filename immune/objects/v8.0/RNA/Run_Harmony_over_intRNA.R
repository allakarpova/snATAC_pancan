...
runHarmonyNormalization <- function(obj, dims=30, column = 'Piece_ID_RNA') {
  ...
  obj <- obj %>%
    ...
    FindClusters(verbose = FALSE, resolution = 2,algorithm = 4,
                 method='igraph') %>%
...
p2 <- DimPlot(obj, group.by = "cell_type_v8.0_Tcell", label = TRUE)
ggsave(glue::glue('Dimplot_{add_filename}_cell_type_v8.0_Tcell.pdf'), plot = p2, width = 12, height = 7)
