...
all.rna@meta.data$to_remove <- all.rna$Cancer == 'MM'
...
all.rna.list <- lapply(X = all.rna.list, FUN = function(x) {
  ...
  if(opt$regress.cc.interferon.genes) {
    interferon.genes <- fread('/diskmnt/Projects/snATAC_analysis/immune/cell_typing/v8.0/allRNA2/by_lineage/no_doublets/Int_chemistry_cancer_reference_multiome2/no_doublets2/Interferon.reponse.genes.tsv', data.table = F, header = T) %>%
      pull(V1)
    x <- GetResidual(x, features = c(interferon.genes, cc.genes.updated.2019$s.genes, cc.genes.updated.2019$g2m.genes))
    dim(x@assays$SCT@scale.data)
  } 
  #DefaultAssay(x) <- 'SCT'
  return(x)
})
...
saveRDS(rna.anchors, 'Integration_anchors.rds')
...
int <- FindClusters(int, resolution = 1, algorithm = 4,
                    method='igraph')
...
cat('saving the object...\n')
saveRDS(int,   paste0("PanImmune_integrated_allRNA_by_",int_batch,"_",add_filename,".rds"))

dimplot=DimPlot(object = int, label = TRUE) + NoLegend()
pdf(paste0("Dimplot_integrated_allRNA_by_",int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
print(dimplot)
dev.off()

dimplot=DimPlot(object = int, label = F, group.by = "Piece_ID")
pdf(paste0("Dimplot_Piece_ID_integrated_allRNA_by_",int_batch,"_",add_filename,".pdf"),height=12,width=25, useDingbats = F)
print(dimplot)
dev.off()

dimplot=DimPlot(object = int, label = F, group.by = "data.type.rna")
pdf(paste0("Dimplot_data.type_integrated_allRNA_by_",int_batch,"_",add_filename,".pdf"),height=12,width=14, useDingbats = F)
print(dimplot)
dev.off()

dimplot=DimPlot(object = int, label = TRUE, group.by = "cell_type.harmonized.cancer")
pdf(paste0("Dimplot_cell_type.harmonized.cancer_integrated_allRNA_by_",int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
print(dimplot)
dev.off()

dimplot=DimPlot(object = int, label = TRUE, group.by = cell_column)
pdf(paste0("Dimplot_",cell_column,"_integrated_allRNA_by_",int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
print(dimplot)
dev.off()

dimplot=DimPlot(object = int, label = TRUE, group.by = "Cancer")
pdf(paste0("Dimplot_Cancer_integrated_allRNA_by_",int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
print(dimplot)
dev.off()
