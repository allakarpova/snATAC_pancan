...
DefaultAssay(stroma.obj) <- 'RNA'
all.rna <- all.rna %>%  DietSeurat(assay = 'RNA', counts = TRUE, data = TRUE)
all.rna <- subset(all.rna, Cancer=='MM', invert = TRUE) #remove MM 
stroma.obj <- stroma.obj %>%  DietSeurat(assay = 'RNA', counts = TRUE, data = TRUE)
dim(all.rna)
all.rna <- merge(all.rna, stroma.obj)
dim(all.rna)
...
saveRDS(int,   paste0("PanImmune_stroma_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".rds"))
