## Alla Karpova
### recluster T-cells dont remove IFN genes and don't regress out cell cycle, remove Mito genes, remove cell cycle genes
...
all.rna.whole <- readRDS(input.path)
...
if (!is.null(meta.path)) {
  ...
  all.rna.whole <- AddMetaData(all.rna.whole, metadata = my.metadata)
}
all.rna.whole@meta.data$to_remove <- grepl('oublet', as.character(unlist(all.rna.whole[[cell_column]])))
DefaultAssay(all.rna.whole) <- 'RNA'
all.rna.whole <- all.rna.whole %>%  DietSeurat(assay = 'RNA', counts = TRUE, data = TRUE)
dim(all.rna.whole)
all.rna.whole <- subset(x = all.rna.whole, subset = to_remove, invert = TRUE)
dim(all.rna.whole)
...
conditions %>% walk (function(column) {
  ...
  trash.genes <- tumor.genes %>% 
    filter(p.vals<0.1, pct.tumor.fake>0, Cancer.rank < 6) %>% 
    pull(features.plot) %>% 
    unique()
  ...
  genes.i.dont.want2 <- Reduce(union, list(mito.genes, trash.genes, cellcycle.genes))
  ...
  int <- RunUMAP(int, reduction = "pca", dims = 1:50)
  ...
  saveRDS(int,   paste0("PanImmune_",column,"_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".rds"))
  
  dimplot=DimPlot(object = int, label = TRUE) + NoLegend()
  pdf(paste0("Dimplot_",column,"_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = F, group.by = "Piece_ID_RNA")
  pdf(paste0("Dimplot_",column,"_Piece_ID_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=25, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = F, group.by = "data.type.rna")
  pdf(paste0("Dimplot_",column,"_data.type_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=14, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = "cell_type.harmonized.cancer")
  pdf(paste0("Dimplot_",column,"_cell_type.harmonized.cancer_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = cell_column)
  pdf(paste0("Dimplot_",column,"_",cell_column,"_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = "Cancer")
  pdf(paste0("Dimplot_",column,"_Cancer_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
