...
all.rna$is_Tcell_lin <- grepl('T-cell|NK|Treg|MAIT|ILC|CD8|CD4', as.character(unlist(all.rna[[cell_column]])))
all.rna$is_Bcell_lin <- grepl('B-cell|Plasma', as.character(unlist(all.rna[[cell_column]])))
all.rna$is_Myeloid_lin <- grepl('Macro|DC|Mast|Microgl|Mono|Neutro', as.character(unlist(all.rna[[cell_column]])))
...
conditions %>% walk (function(column) {
  ...
  all.rna.sub@meta.data$Data.source <- ifelse(all.rna.sub$Cancer == 'PBMC', '10x', 'DingLab')
  all.rna.sub@meta.data$Batches <- case_when(all.rna.sub$Cancer %in% c('PBMC') ~ paste(all.rna.sub$Cancer, all.rna.sub$Chemistry, sep = '__'),
  ...
  pdf(paste0("Dimplot_", column,"_Piece_ID_",add_filename,".pdf"),height=12,width=25, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = F, group.by = "data.type.rna")
  pdf(paste0("Dimplot_", column,"_data.type_",add_filename,".pdf"),height=12,width=14, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = "cell_type.harmonized.cancer")
  pdf(paste0("Dimplot_", column,"_cell_type.harmonized.cancer_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = cell_column)
  pdf(paste0("Dimplot_", column,"_",cell_column,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = "Cancer")
  pdf(paste0("Dimplot_", column,"_Cancer_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
