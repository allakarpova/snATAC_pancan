#Alla wrote the script
#v2 added input parameter and removed dependency on cell types
...
option_list = list(
  make_option(c("-i", "--input"),
              type="character",
              default=NULL, 
              help="Input file with paths to xenium objects",
              metavar="character"),
...
xenium.tb <- fread(in_path, header = T, data.table=F) %>%
...
pwalk(unname(as.list(xenium.tb)), function( sample,cancer,pathf) {
  if(!file.exists(glue::glue('{sample}_Niches_banksy_lambda{lambda.f}_k_geom{k_geom.f}_res_0.1_0.2_0.5.tsv'))) {
    ...
    VariableFeatures(obj) <- VariableFeatures(obj)[!grepl(":|-WT|acter|coli|papil", VariableFeatures(obj))]
    obj <- ScaleData(obj)
