...
option_list = list(
  ...
  make_option(c("-c", "--cancer"),
              type="character",
              default="HNSCC", 
              help="which cancer type to merge",
              metavar="character")
...
samples.cancer <- base::subset(samples, `Disease Type`==cancer.type)
...
if (file.exists(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_normalized_",add_filename,".rds"))) {
  cat('opening normalized new peaks object...\n')
  combined <- readRDS(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_normalized_",add_filename,".rds"))
} else {
  if (file.exists(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_not_normalized_",add_filename,".rds"))) {
    cat('opening not normalized new peaks object...\n')
    combined <- readRDS(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_not_normalized_",add_filename,".rds"))
  } else {
    if (file.exists(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_not_normalized_",add_filename,".rds"))) {
      cat('opening not normalized old peaks object...\n')
      combined <- readRDS(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_not_normalized_",add_filename,".rds"))
    } else {
      paths <- NULL
      ...
      saveRDS(combined, paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_not_normalized_",add_filename,".rds"))
    }
    
    #my peaks predone by Merge_ATAC_samples_auto_v.5.4 script these are pancancer peaks
    all_peaks.filtered <- fread( paste0('peaks/139_All_recentered_final.',add_filename,'.tsv'), data.table = T)
    ...
    cat('saving the object...\n')
    saveRDS(combined, paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_not_normalized_",add_filename,".rds"))
  }
  ...
  saveRDS(combined, paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_normalized_",add_filename,".rds"))
  
}
#update metadata
metadata <- fread(paste0('/diskmnt/Projects/snATAC_primary/04_celltyped_rds/v4.0/',cancer.type, '_',length(samples.id) ,'_samples_metadata_data_freeze_v2.0.tsv')) %>% 
...
ggsave(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_sample_", add_filename, ".pdf"),plot = p1,height=12,width=17, useDingbats = F)

p1 <- DimPlot(combined, group.by = 'dataset', pt.size = 0.1)
ggsave(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_dataset_", add_filename, ".pdf"),plot = p1,height=12,width=17, useDingbats = F)

p2 <- DimPlot(combined, pt.size = 0.1,label=T)
ggsave(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_cluster_", add_filename, ".pdf"), plot = p2, height=12,width=14, useDingbats = F)
...
ggsave(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_cell_type.harm_", add_filename, ".pdf"), plot = p3,height=12,width=15, useDingbats = F)

p4 <- DimPlot(combined, pt.size = 0.1, label=T, group.by = 'data.type', cols = 'Paired')
ggsave(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_data.type_", add_filename, ".pdf"), plot = p4,height=12,width=13, useDingbats = F)

p5 <- DimPlot(combined, pt.size = 0.1, label=T, group.by = 'Sample_type', cols = 'Paired')
ggsave(paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_Sample_type_", add_filename, ".pdf"), plot = p5,height=12,width=13, useDingbats = F)

write.table(combined@meta.data, paste0(cancer.type,'_', length(samples.id),"_snATAC_Merged_new_peaks_normalized_",add_filename,"_metaData.txt"),sep="\t",quote=FALSE, row.names = T)
write.table(samples.id,paste0(cancer.type,'_', "Samples_snATAC_Merged_",add_filename,".txt"),sep="\t",quote=FALSE)
