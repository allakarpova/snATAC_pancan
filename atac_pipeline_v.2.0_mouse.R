#Created by Reyka Jayasinghe (reyka@wustl.edu), updated by Nadezhda Terekhanova (nvterekhanova@wustl.edu)
...
for (sample in samples){
	...
	###########################################################
	############Adding annotations for mm10 to the object######
	###########################################################
	...
	pbmc$nucleosome_group <- ifelse(pbmc$nucleosome_signal > opt$ns_max, paste('NS >', opt$ns_max), paste('NS <', opt$ns_max))
	fragment_period_plot=FragmentHistogram(object = pbmc, group.by = 'nucleosome_group', region = 'chr1-1-10000000')	
	...
	pbmc <- subset(
	x = pbmc,
	subset = peak_region_fragments > opt$prf_min &
	peak_region_fragments < opt$prf_max &
