...
c('BRCA','HNSCC', 'CRC', 'CESC', 'PDAC', 'OV', 'UCEC') %>% walk(function(cancer) {
  ...
  all.peaks.with.links <- all.links.no.gained.tumor.normal %>% pull(peak) %>% unique
  ...
  peaks.matched <- MatchRegionStats(
    meta.feature = meta.feature[all.peaks.with.links, ],
  ...
  fwrite(enriched.motifs, glue::glue('Enriched.motifs.{cancer}.bckgr.All.peaks.with.links.GC.tsv'), sep='\t', row.names = F, col.names = T)
