...
driver.piece.anno  <- genomic.data %>% dplyr::select('Piece_ID', 'KMT2D', "KMT2C")
...
cells.trunc <- obj@meta.data %>% 
  ...
  dplyr::filter(grepl('Nonsen|Shift', KMT2D) | grepl('Nonsen|Shift', KMT2C)) %>% 
  rownames

mutated.cancers <- obj@meta.data %>% 
  filter(cell_type.normal == 'Tumor') %>%
  filter(!(Piece_ID %in% exclude)) %>%
  dplyr::filter(grepl('Nonsen|Shift', KMT2D) | grepl('Nonsen|Shift', KMT2C)) %>% 
  pull(Cancer.new) %>% 
...
print(mutated.cancers)

cells.wt <- obj@meta.data %>% 
  filter(cell_type.normal == 'Tumor') %>%
  filter(!(Piece_ID %in% exclude)) %>%
  filter(KMT2D == 'WT' & KMT2C == 'WT' ) %>% 
  filter(Cancer.new %in% mutated.cancers) %>% # ditch not mutated cancers
  rownames()

obj <- subset(obj, cells = c(cells.trunc, cells.wt))
...
da_peaks$Driver='KMT2C/D'
da_peaks$peak=rownames(da_peaks)

fwrite(da_peaks, glue::glue("KMT2C.D_Mut_vs_WT.Pooled.pct.0.05.fCh.0.1.20230404_downsampled200_limited_cancers.tsv"),
       sep="\t",row.names=FALSE)
