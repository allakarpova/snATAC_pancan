####2021-07-02
# Alla fixed QC columns naming to remove duplicated information
# Alla changed to filtered matrices, added keepStandardChromosomes and remove blacklist peaks using subsetByOverlaps
#Modified by Nadezhda Terekhanovaby Nadezhda Terekhanova (nvterekhanova@wustl.edu) using updated peaak calling with MACS2
...
for (sample in samples){
	...
	outputpath=paste(sample,"/",sep="")
	dir.create(outputpath)

	counts <- Read10X_h5(paste(atac_data_folder,"/outs/filtered_peak_bc_matrix.h5",sep=""))
#	counts <- Read10X_h5(paste("/diskmnt/Projects/ccRCC_scratch/ccRCC_snATAC/Resources/snATAC_Processed_Data/Signac.1.0.0/1.Create_rds/raw_peak_bc_matrix/",sample,"_raw_peak_bc_matrix.h5",sep=""))
	metadata <-read.csv(file=paste(atac_data_folder,"/outs/singlecell.csv",sep=""), header = TRUE, row.names = 1)
	fragment.path <- paste(atac_data_folder,"/outs/fragments.tsv.gz",sep="")
...
quote=FALSE,row.names=FALSE)
	...
	pbmc <- AddMetaData(object = pbmc, metadata = frip, col.name = 'pct_read_in_peaks_500MACS2')
	pbmc <- AddMetaData(object = pbmc, metadata = peak.counts, col.name = 'peak_region_fragments_500MACS2')
	...
	pbmc$blacklist_ratio <- pbmc$blacklist_region_fragments / pbmc$peak_region_fragments_500MACS2
	...
	QC_plot=VlnPlot(
 	  object = pbmc,
	  features = c('pct_read_in_peaks_500MACS2', 'peak_region_fragments_500MACS2',
	...
	QC_plot=VlnPlot(
          object = pbmc,
          features = c('pct_reads_in_peaks', 'peak_region_fragments_500MACS2',
               'TSS.enrichment', 'blacklist_ratio', 'nucleosome_signal'),
          pt.size = 0.1,
          ncol = 5
        )

        pdf(paste(outputpath,"/",sample,"_1_AfterQC.pdf",sep=""),height=6,width=12)
        print(QC_plot)
        dev.off()
	...
	pdf(paste(outputpath,"/",sample,"_DepthCorrelation_2_QC.pdf",sep=""),height=6,width=12)
	...
	pdf(paste(outputpath,"/",sample,"_3_Dimplot.pdf",sep=""),height=6,width=6)
	print(dimplot)
	dev.off()
