...
for (i in 1:length(samples.id)){
  ...
  if (!file.exists(Fragments(atac[[i]])[[1]]@path)) stop("Urgh, this sample object can't locate fragments file")
...
#save individual objects with the common set of peaks
for (i in 1:length(samples.id)){
  saveRDS(atac[[i]], paste0('indiv_obj/',samples.id, '_atac_common_peaks_', add_filename,'.rds'))
}
...
saveRDS(combined, paste0(length(samples.id),"_snATAC_Merged_not_clustered_",add_filename,".rds"))
