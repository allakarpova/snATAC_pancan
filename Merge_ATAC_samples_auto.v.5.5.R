...
if(file.exists(paste0('peaks/', length(samples.id),'_All_recentered_final.',add_filename,'.tsv'))) {
  ...
  rm(recentered_final.cancer, all_peaks.cancer)
  gc()
  ...
  recentered_final.cancer.filtered.pancan <- foreach(x=recentered_final.cancer.filtered) %do% {
  ...
  pancan_all_peaks <- rbindlist(recentered_final.cancer.filtered.pancan)
