...
if (length(regular.sample) > 0) {
  ...
  rownames(regular.metadata) <- paste(regular.metadata$Sample, regular.metadata$V1, sep = '_')
} else {
...
if (length(combo.sample) > 0) {
  ...
  combo.metadata <- lapply(combo.sample, function(x) {
    caseid <- samples$`Case ID`[samples$Sample==x]
    pieceid <- samples$Piece_ID[samples$Sample==x]
    type <- samples$`Sample Type`[samples$Sample==x]
    toret <- fread(meta.files[grepl(x,meta.files)], data.table = F) %>%
  ...
  rownames(combo.metadata) <- paste(combo.metadata$Sample, combo.metadata$V1, sep = '_')
  ...
  if (length(regular.sample) > 0) {
    ...
    total.metadata <- rbind(cbind(regular.metadata[common.columns], data.type = 'snATAC'), 
                            cbind(combo.metadata[common.columns], data.type = '10x_SC_Multi_ATAC_SEQ'))
  } else {
    total.metadata <- cbind(combo.metadata, data.type = '10x_SC_Multi_ATAC_SEQ')
  }
  
} else {
  total.metadata <- cbind(regular.metadata, data.type = 'snATAC')
}
...
if(cancer.type == 'BRCA') {
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial|LSEC', cell_type) ~ 'Endothelial',
                                                                                ...
                                                                                cell_type =='B' | grepl('B-cells|Plasma', cell_type) ~ 'B-cells/Plasma',
  ...
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial|Endo', cell_type) ~ 'Endothelial',          
                                                              ...
                                                              grepl('B-cells|Plasma', cell_type) ~ 'B-cells/Plasma',
                                                              grepl('Macro', cell_type) ~ 'Macrophages',
                                                              grepl('DC', cell_type) ~ 'DC',
                                                              grepl('NK', cell_type) ~ 'NK',
  ...
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial', cell_type) ~ 'Endothelial',
                                                                                ...
                                                                                grepl('Malignant', cell_type) ~ 'Tumor',
                                                                                grepl('NK', cell_type) ~ 'NK',
                                                                                grepl('Mast', cell_type) ~ 'Mast',
  ...
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial', cell_type) ~ 'Endothelial',
                                                                                grepl('CD4|CD8|T-cells', cell_type) ~ 'T-cells',
                                                                                cell_type =='B' | grepl('preB', cell_type) ~ 'B-cells/Plasma',
                                                                                cell_type =='Plasma' & Sample_type == 'Normal' ~ 'B-cells/Plasma',
  ...
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial', cell_type) ~ 'Endothelial',
                                                                                grepl('CD4|CD8|T-cells', cell_type) ~ 'T-cells',
                                                                                grepl('Treg', cell_type) ~ 'Tregs',
                                                                                cell_type =='B-cells' | grepl('Plasma', cell_type) ~ 'B-cells/Plasma',
                                                                                grepl('DC', cell_type) ~ 'DC',
                                                                                grepl('umor', cell_type) ~ 'Tumor',
                                                                                grepl('ibroblast', cell_type) ~ 'Fibroblasts',
                                                                                grepl('Macrop|Mono', cell_type) ~ 'Macrophages',
  ...
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial', cell_type) ~ 'Endothelial',
                                                                                ...
                                                                                grepl('Treg', cell_type) ~ 'Tregs',
                                                                                grepl('Lymph', cell_type) ~ 'Lymphocytes',
                                                                                cell_type =='B-cells' | cell_type =='B' | grepl('Plasma', cell_type) ~ 'B-cells/Plasma',
                                                                                grepl('DC', cell_type) ~ 'DC',
                                                                                grepl('umor', cell_type) ~ 'Tumor',
                                                                                grepl('CAF|Fibroblast', cell_type) ~ 'Fibroblasts',
...
fwrite(total.metadata, paste0(cancer.type,'_',length(samples.id),'_samples_metadata_data_freeze_',v,'.tsv'), sep = '\t', row.names = T)
