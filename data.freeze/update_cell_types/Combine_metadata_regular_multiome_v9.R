...
if(cancer.type == 'BRCA') {
  ...
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial', cell_type) ~ 'Endothelial',
                                                                                ...
                                                                                grepl('Mast', cell_type) ~ 'Mast',
                                                                                grepl('Epithelial', cell_type) ~ 'Normal epithelial cells',
                                                                                grepl('squamous', cell_type) ~ 'Normal squamous epithelial cells',
                                                                                grepl('Erythrocyte', cell_type) ~ 'Erythrocytes',
                                                                                TRUE ~ cell_type))
} else if (cancer.type == 'HNSCC') {
  total.metadata <- total.metadata %>% mutate(cell_type.harmonized = case_when (grepl('Endothelial|Endotelial', cell_type) ~ 'Endothelial',
                                                                                grepl('CD4|CD8|T-cells', cell_type) ~ 'T-cells',
                                                                                grepl('Treg', cell_type) ~ 'Tregs',
                                                                                ...
                                                                                grepl('Mast', cell_type) ~ 'Mast',
                                                                                grepl('Pericyte', cell_type) ~ 'Pericytes',
                                                                                grepl('pithelial', cell_type) ~ 'Normal epithelial cells',
                                                                                grepl('Erythrocyte', cell_type) ~ 'Erythrocytes',
                                                                                TRUE ~ cell_type))
...
fwrite(total.metadata, paste0(cancer.type,'_',length(samples.id),'_samples_metadata_v9_data_freeze_',v,'.tsv'), sep = '\t', row.names = T)
