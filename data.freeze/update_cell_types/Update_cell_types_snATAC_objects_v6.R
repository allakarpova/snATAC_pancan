...
for (i in 1:nrow(samples)) {
  ...
  print(s)
  if (!do.all.over) {
    if (file.exists(paste0(s,'_upd_v6_cellTyped.meta.data'))) { # dont do updating if its already updated
      next()
    }
  }
  ...
  ggsave(paste0(s,'_before_update_to_v6.pdf'), width = 12, height = 6, useDingbats = F)
  ...
  fwrite(input.obj@meta.data, paste0(s,'_upd_v6_cellTyped.meta.data'), sep = '\t', row.names = T)
