...
rna.path <- paste0('/diskmnt/Projects/snATAC_primary/PanCan_ATAC_data_freeze/v5.0/snRNA/Merged_objects/', cancer, '/', cancer, '_merge_obj_processed-with_doublets.rds')
atac.path <- paste0('/diskmnt/Projects/snATAC_primary/PanCan_ATAC_data_freeze/v5.0/snATAC/Merged_objects_PanCan_peaks/RDS.withUMAPs.50PCs.noDoublets/', cancer, '_snATAC_Merged.PancanSet.noDoublets.20220209.rds')
...
rna.meta.renamed <- rna.meta %>% 
  dplyr::select(-orig.ident, -Piece_ID, -UMAP_1, -UMAP_2, -data.type, -Cancer) %>% 
...
atac.meta.renamed <- atac.meta %>% 
  ...
  rename(cell_type.harmonized.cancer.atac = cell_type.harmonized.cancer, 
...
if (file.exists(paste0('out/', cancer, '_all_cells_multiome_obj.20220325.rds'))) {
  r.obj <- readRDS(paste0('out/', cancer, '_all_cells_multiome_obj.20220325.rds'))
} else {
  ...
  rna.cells <- rownames(rna.meta %>% filter(data.type == '10x_SC_Multi_ATAC_SEQ' & Cancer == cancer))
  atac.cells <- rownames(atac.meta %>% filter(data.type == '10x_SC_Multi_ATAC_SEQ' & Cancer == cancer))
  combo.cells <- rownames(combo.meta %>% filter(data.type == '10x_SC_Multi_ATAC_SEQ' & Cancer == cancer))
