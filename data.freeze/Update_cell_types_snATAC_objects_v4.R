...
for (i in 1:nrow(samples)) {
  ...
  possible.files <- list.files(path = '/diskmnt/Projects/snATAC_primary/04_celltyped_rds/v3.1', pattern = s, full.names = T, recursive = T)
  ...
  if (s == "PDAC_TWCE-HT-018-P-Slice2_ATAC-lib1") { #handle a special case
  ...
  if (samples$`Data Type`[i]=='snATAC') {
    if (!is.updated.version) { #move predicted.id column to cell types if the metadata has not been previously updated but it is really does not matter
      input.metadata$cell_type <- input.metadata$predicted.id
    }
    ...
    DimPlot(input.obj, label = T) + DimPlot(input.obj, group.by = 'cell_type', label = T, cols = 'Paired') # plot
    ggsave(paste0(s,'_before_update_to_v4.pdf'), width = 12, height = 6, useDingbats = F)
    #update metadata
    ...
    DimPlot(input.obj, label = T) + DimPlot(input.obj, group.by = 'cell_type', label = T, cols = 'Paired')
    ggsave(paste0(s,'_upd_celltypes.pdf'), width = 12, height = 6, useDingbats = F)
    fwrite(input.obj@meta.data, paste0(s,'_upd_cellTyped.meta.data'), sep = '\t', row.names = T)
