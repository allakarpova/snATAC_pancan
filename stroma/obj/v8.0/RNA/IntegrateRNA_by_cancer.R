## Alla Karpova
### Integrate RNA stroma cells by cancer type
...
unique(all.rna$Cancer) %>% walk(function(can) {
  ...
  saveRDS(int,   paste0("Stroma_", can,"_integrated_allRNA_by_",opt$int_batch,"_",add_filename,".rds"))
  
  dimplot=DimPlot(object = int, label = TRUE) + NoLegend()
  pdf(paste0("Dimplot_integrated_allRNA_", can,"_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = F, group.by = "Piece_ID_RNA")
  pdf(paste0("Dimplot_Piece_ID_integrated_", can,"_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=25, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = F, group.by = "data.type.rna")
  pdf(paste0("Dimplot_data.type_integrated_", can,"_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=14, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = "cell_type.harmonized.cancer")
  pdf(paste0("Dimplot_cell_type.harmonized.cancer_integrated_", can,"_allRNA_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = cell_column)
  pdf(paste0("Dimplot_",cell_column,"_integrated_allRNA_", can,"_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
  
  dimplot=DimPlot(object = int, label = TRUE, group.by = "Cancer")
  pdf(paste0("Dimplot_Cancer_integrated_allRNA_", can,"_by_",opt$int_batch,"_",add_filename,".pdf"),height=12,width=15, useDingbats = F)
  print(dimplot)
  dev.off()
})
