...
if(!file.exists(paste0('PanStroma_merged_allRNA_normalized_', add_filename, '.rds'))) {
  ...
  my.metadata$is_notCancer <- !grepl('Tumor', as.character(unlist(my.metadata[[cell_column]])))
  ...
  rna <- map(paths, function(p) {
    ...
    cat('subsetting\n')
    obj$is_notCancer <- !grepl('Tumor', as.character(unlist(obj[[cell_column]])))
    cat('subsetting\n')
    obj.my <- subset(x = obj, subset = is_notCancer)
  ...
  saveRDS(all.rna, paste0('PanNotCancer_merged_allRNA_', add_filename, '.rds'))
  
  cat ('Normalizing merged all RNA object\n')
  all.rna <- runAllNormalization (all.rna, dims = 30)
  ################
  
  saveRDS(all.rna, paste0('PanNotCancer_merged_allRNA_normalized_', add_filename, '.rds'))
} else {
  all.rna <- readRDS(paste0('PanNotCancer_merged_allRNA_normalized_', add_filename, '.rds'))
}
...
saveRDS(int,   paste0("PanNotCancer_integrated_object_allRNA_",add_filename,".rds"))
