## Alla Karpova
### merge regular RNA data for stroma cells and integrate with seuratv8.0 data freeze
...
samples$Keep <- samples$`Include in stroma` %>% unlist()
...
cancers <- c('BRCA', 'ccRCC', 'GBM', 'CRC', 'HNSCC', 'CESC', 'OV', 'UCEC', "PDAC", "SKCM")
...
if(!file.exists(paste0('PanStroma_merged_allRNA_normalized_', add_filename, '.rds'))) {
  
  my.metadata <- fread(meta.path, data.table = F) %>% 
    data.frame(row.names = 1, check.rows = F, check.names = F)
  my.metadata$is_stroma <- grepl('Endoth|Pericy|Fibro|vSMC|Lymphatic', as.character(unlist(my.metadata[[cell_column]])))
  #cancers.with.stroma <- my.metadata %>% filter(is_stroma) %>% pull(Cancer)
  #cancers.with.stroma <- cancers.with.stroma[-which(cancers.with.stroma=='OV')] #remove ovarian cancer because the only snRNA sample with stroma cells is weird VF031V1-Tm1Y1
  #merged.obj.path <- merged.obj.path[names(merged.obj.path) %in% cancers.with.stroma]
  ...
  rna <- map(paths, function(p) {
    ...
    cat('subsetting\n')
    obj$is_stroma <- grepl('Endoth|Pericy|Fibro|vSMC|Lymphatic', as.character(unlist(obj[[cell_column]])))
    cat('subsetting\n')
  ...
  saveRDS(all.rna, paste0('PanStroma_merged_allRNA_', add_filename, '.rds'))
  
  cat ('Normalizing merged all RNA object\n')
  all.rna <- runAllNormalization (all.rna, dims = 30)
  ################
  
  saveRDS(all.rna, paste0('PanStroma_merged_allRNA_normalized_', add_filename, '.rds'))
} else {
  all.rna <- readRDS(paste0('PanStroma_merged_allRNA_normalized_', add_filename, '.rds'))
}


all.rna@meta.data$Batches <- all.rna$Chemistry
...
saveRDS(int,   paste0("PanStroma_integrated_object_allRNA_",add_filename,".rds"))
